...
class SrShapeBurstFront extends RevSliderFunctions {
	...
	public function get_script_list(){
		if (file_exists(RS_SHAPEBURST_PLUGIN_PATH . 'public/js/' . $this->slug . '.js')) {
			return array(
				'shapeburst'	=> RS_SHAPEBURST_PLUGIN_URL.'public/js/' . $this->slug . '.js',
				'sb_SBurst'		=> RS_SHAPEBURST_PLUGIN_URL.'public/js/sburst.js',
				'sb_Tools'		=> RS_SHAPEBURST_PLUGIN_URL.'public/js/tools.js',
				'sb_Shaders'	=> RS_SHAPEBURST_PLUGIN_URL.'public/js/shaders.js'
			);
		}else{
			return array('shapeburst' => RS_SHAPEBURST_PLUGIN_URL.'public/js/' . $this->slug . '.min.js');
		}
	}

	public function get_addition_list($full = true){
		$path = ($full === true) ? 'SR7.E.resources.' : '';
		return array($path.'sbURL' => RS_SHAPEBURST_PLUGIN_URL);
	}
	...
	public function enqueue_scripts(){
		...
		$this->script_enqueued = true;
		if (file_exists(RS_SHAPEBURST_PLUGIN_PATH . 'public/js/' . $this->slug . '.js')) {
			wp_enqueue_script('revslider-'.$this->slug.'-addon', RS_SHAPEBURST_PLUGIN_URL."public/js/" . $this->slug . ".js", '', RS_REVISION, array('strategy' => 'async'));
			wp_enqueue_script('revslider-'.$this->slug.'-addon-sb_SBurst', RS_SHAPEBURST_PLUGIN_URL . "public/js/sburst.js", '', RS_REVISION, array('strategy' => 'async'));
			wp_enqueue_script('revslider-'.$this->slug.'-addon-sb_Tools', RS_SHAPEBURST_PLUGIN_URL . "public/js/tools.js", '', RS_REVISION, array('strategy' => 'async'));
			wp_enqueue_script('revslider-'.$this->slug.'-addon-sb_Shaders', RS_SHAPEBURST_PLUGIN_URL."public/js/shaders.js", '', RS_REVISION, array('strategy' => 'async'));
		} else {
			wp_enqueue_script('revslider-'.$this->slug.'-addon', RS_SHAPEBURST_PLUGIN_URL . "public/js/" . $this->slug . ".min.js", '', RS_REVISION, array('strategy' => 'async'));
		}
	}

	public function write_footer_scripts(){
		echo '<script type="text/javascript">window.RVS = window.RVS || {}; window.RVS.ENV = window.RVS.ENV || {}; window.RVS.ENV.SHAPEBURST_URL = "'.RS_SHAPEBURST_PLUGIN_URL.'";</script>'."\n";
	}
	...
	public function add_addon_files($html, $export){
		...
		$dynamic_files = array(
			RS_SHAPEBURST_PLUGIN_PATH.'public/assets/' => 'revslider-'.$this->slug.'-addon/public/assets/',
		);
		...
		foreach($list ?? [] as $handle => $script){
			$script_path = str_replace(RS_SHAPEBURST_PLUGIN_URL, RS_SHAPEBURST_PLUGIN_PATH, $script);
			if(!$export->usepcl){
				$export->zip->addFile($script_path, $export_path_js . $handle . '.js');
			}else{
				$export->pclzip->add($script_path, PCLZIP_OPT_REMOVE_PATH, RS_SHAPEBURST_PLUGIN_PATH.'public/js/', PCLZIP_OPT_ADD_PATH, $export_path_js);
			}
