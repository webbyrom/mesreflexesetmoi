...
class SrBubbleMorphFront extends RevSliderFunctions {
	...
	public function add_addon_files($html, $export){
		...
		foreach($list ?? [] as $handle => $script){
			$script_path = str_replace(RS_BUBBLEMORPH_PLUGIN_URL, RS_BUBBLEMORPH_PLUGIN_PATH, $script);
			if(!$export->usepcl){
				$export->zip->addFile($script_path,  $export_path_js . $handle . '.js');
			}else{
				$export->pclzip->add($script_path, PCLZIP_OPT_REMOVE_PATH, RS_BUBBLEMORPH_PLUGIN_PATH.'public/js/', PCLZIP_OPT_ADD_PATH, $export_path_js);
			}
