...
class SrParticlesFront extends RevSliderFunctions {
	...
	public function get_script_list(){
		$min = file_exists(RS_PARTICLES_PLUGIN_PATH . 'public/js/' . $this->slug . '.js') ? '' : '.min';
		if($min === '.min'){
			return array('particles' => RS_PARTICLES_PLUGIN_URL.'public/js/' . $this->slug . $min . '.js');
		}else{
			return array(
				'particles' => RS_PARTICLES_PLUGIN_URL.'public/js/' . $this->slug . $min . '.js',
				'pjs'		=> RS_PARTICLES_PLUGIN_URL.'public/js/pjs.js'
			);
		}
	}
	...
	public function enqueue_scripts(){
		...
		$min = file_exists(RS_PARTICLES_PLUGIN_PATH . 'public/js/' . $this->slug . '.js') ? '' : '.min';
		if($min === '.min'){
			wp_enqueue_script('revslider-'.$this->slug.'-addon', RS_PARTICLES_PLUGIN_URL . "public/js/" . $this->slug . $min . ".js", '', RS_REVISION, array('strategy' => 'async'));
		} else {
			wp_enqueue_script('revslider-pjs-lib', RS_PARTICLES_PLUGIN_URL . "public/js/pjs.js", '', RS_REVISION, ['strategy' => 'async']);
			wp_enqueue_script('revslider-'.$this->slug.'-addon', RS_PARTICLES_PLUGIN_URL . "public/js/" . $this->slug . $min . ".js", '', RS_REVISION, array('strategy' => 'async'));
		}
	}
	...
	public function add_addon_files($html, $export){
		...
		foreach($list ?? [] as $handle => $script){
			$script_path = str_replace(RS_PARTICLES_PLUGIN_URL, RS_PARTICLES_PLUGIN_PATH, $script);
			if(!$export->usepcl){
				$export->zip->addFile($script_path,  $export_path_js . $handle . '.js');
			}else{
				$export->pclzip->add($script_path, PCLZIP_OPT_REMOVE_PATH, RS_PARTICLES_PLUGIN_PATH.'public/js/', PCLZIP_OPT_ADD_PATH, $export_path_js);
			}
