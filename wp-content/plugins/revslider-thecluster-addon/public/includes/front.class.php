...
class SrTheClusterFront extends RevSliderFunctions {
	...
	public function get_script_list(){
		$min = file_exists(RS_THECLUSTER_PLUGIN_PATH . 'public/js/' . $this->slug . '.js') ? '' : '.min';
		if($min === '.min'){
			return array('thecluster' => RS_THECLUSTER_PLUGIN_URL.'public/js/' . $this->slug . $min . '.js');
		}else{
			return array(
				'thecluster'	=> RS_THECLUSTER_PLUGIN_URL.'public/js/' . $this->slug . $min . '.js',
				'tc_Cluster'	=> RS_THECLUSTER_PLUGIN_URL.'public/js/cluster.js',
				'tc_Shaders'	=> RS_THECLUSTER_PLUGIN_URL.'public/js/shaders.js',
				'tc_Tools'		=> RS_THECLUSTER_PLUGIN_URL.'public/js/tools.js',
			);
		}
	}
	...
	public function get_addition_list($full = true){
		...
		return array($path.'tcURL' => RS_THECLUSTER_PLUGIN_URL);
	}
	...
	public function enqueue_scripts(){
		...
		if (file_exists(RS_THECLUSTER_PLUGIN_PATH . 'public/js/' . $this->slug . '.js')) {
			wp_enqueue_script('revslider-'.$this->slug.'-addon', RS_THECLUSTER_PLUGIN_URL."public/js/thecluster.js", '', RS_REVISION, array('strategy' => 'async'));
			wp_enqueue_script('revslider-'.$this->slug.'-addon-tc_Cluster', RS_THECLUSTER_PLUGIN_URL . "public/js/cluster.js", '', RS_REVISION, array('strategy' => 'async'));
			wp_enqueue_script('revslider-'.$this->slug.'-addon-tc_Shaders', RS_THECLUSTER_PLUGIN_URL . "public/js/shaders.js", '', RS_REVISION, array('strategy' => 'async'));
			wp_enqueue_script('revslider-'.$this->slug.'-addon-tc_Tools', RS_THECLUSTER_PLUGIN_URL . "public/js/tools.js", '', RS_REVISION, array('strategy' => 'async'));			
		} else {
			wp_enqueue_script('revslider-'.$this->slug.'-addon', RS_THECLUSTER_PLUGIN_URL . "public/js/" . $this->slug . ".min.js", '', RS_REVISION, array('strategy' => 'async'));
		}
	}
	...
	public function add_addon_files($html, $export){
		...
		$dynamic_files = array(
			RS_THECLUSTER_PLUGIN_PATH.'public/lib/' => 'revslider-'.$this->slug.'-addon/public/lib/'
		);
		...
		foreach($list ?? [] as $handle => $script){
			$script_path = str_replace(RS_THECLUSTER_PLUGIN_URL, RS_THECLUSTER_PLUGIN_PATH, $script);
			if(!$export->usepcl){
				$export->zip->addFile($script_path, $export_path_js . $handle . '.js');
			}else{
				$export->pclzip->add($script_path, PCLZIP_OPT_REMOVE_PATH, RS_THECLUSTER_PLUGIN_PATH.'public/js/', PCLZIP_OPT_ADD_PATH, $export_path_js);
			}
