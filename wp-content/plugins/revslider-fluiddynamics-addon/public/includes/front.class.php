...
class SrFluidDynamicsFront extends RevSliderFunctions {
	...
	public function get_script_list(){
		$min = file_exists(RS_FLUIDDYNAMICS_PLUGIN_PATH . 'public/js/' . $this->slug . '.js') ? '' : '.min';
		if($min === '.min'){
			return array('fluiddynamics' => RS_FLUIDDYNAMICS_PLUGIN_URL.'public/js/' . $this->slug . $min . '.js');
		}else{
			return array(
				'fluiddynamics' => RS_FLUIDDYNAMICS_PLUGIN_URL.'public/js/' . $this->slug . '.js',
				'fd_Fluid'		=> RS_FLUIDDYNAMICS_PLUGIN_URL.'public/js/fluid.js',
				'fd_Shaders'	=> RS_FLUIDDYNAMICS_PLUGIN_URL.'public/js/shaders.js',
				'fd_Tools'		=> RS_FLUIDDYNAMICS_PLUGIN_URL.'public/js/tools.js'
			);
		}
	}
	...
	public function enqueue_scripts(){
		...
		$this->script_enqueued = true;
		if (file_exists(RS_FLUIDDYNAMICS_PLUGIN_PATH . 'public/js/' . $this->slug . '.js')) {
			wp_enqueue_script('revslider-'.$this->slug.'-addon', RS_FLUIDDYNAMICS_PLUGIN_URL . "/public/js/" . $this->slug . ".js", '', RS_REVISION, ['strategy' => 'async']);
			wp_enqueue_script('revslider-'.$this->slug.'-addon-fd_Fluid', RS_FLUIDDYNAMICS_PLUGIN_URL . "/public/js/fluid.js", '', RS_REVISION, ['strategy' => 'async']);
			wp_enqueue_script('revslider-'.$this->slug.'-addon-fd_Shaders', RS_FLUIDDYNAMICS_PLUGIN_URL . "/public/js/shaders.js", '', RS_REVISION, ['strategy' => 'async']);
			wp_enqueue_script('revslider-'.$this->slug.'-addon-fd_Tools', RS_FLUIDDYNAMICS_PLUGIN_URL . "/public/js/tools.js", '', RS_REVISION, ['strategy' => 'async']);
		} else {
			wp_enqueue_script('revslider-'.$this->slug.'-addon', RS_FLUIDDYNAMICS_PLUGIN_URL . "public/js/" . $this->slug . ".min.js", '', RS_REVISION, ['strategy' => 'async']);
		}
	}
	...
	public function add_addon_files($html, $export){
		...
		foreach($list ?? [] as $handle => $script){
			$script_path = str_replace(RS_FLUIDDYNAMICS_PLUGIN_URL, RS_FLUIDDYNAMICS_PLUGIN_PATH, $script);
			if(!$export->usepcl){
				$export->zip->addFile($script_path,  $export_path_js . $handle . '.js');
			}else{
				$export->pclzip->add($script_path, PCLZIP_OPT_REMOVE_PATH, RS_FLUIDDYNAMICS_PLUGIN_PATH.'public/js/', PCLZIP_OPT_ADD_PATH, $export_path_js);
			}
